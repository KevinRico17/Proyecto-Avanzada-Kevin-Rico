...
int ContarPacientesValidos(const char* ruta = "Data/pacientes.csv") {
    fstream archivo(ruta, ios::in);
    if (!archivo.is_open()) {
        cout << "No se pudo abrir el archivo de pacientes para conteo.\n";
        return 0;
    }
...
InformacionArchivoPaciente* LeerArchivoPacientes(int &cantidad, const char* ruta = "Data/pacientes.csv") {
    cantidad = 0;

   
    int total = ContarPacientesValidos(ruta);
    if (total <= 0) {
        cout << "No hay pacientes válidos para cargar.\n";
        return nullptr;
    }

   
    InformacionArchivoPaciente* pacientes = new InformacionArchivoPaciente[total];

    
    fstream archivo(ruta, ios::in);
    if (!archivo.is_open()) {
        cout << "No se pudo abrir el archivo de pacientes para lectura.\n";
        delete[] pacientes;
        return nullptr;
    }
    ...
    while (archivo.getline(linea, sizeof(linea)) && idx < total) {
        char tmp[512];
        strncpy(tmp, linea, sizeof(tmp) - 1);
        tmp[sizeof(tmp) - 1] = '\0';
        trim(tmp);
        ...
        pacientes[idx++] = CadenasAEstructuraPacientes(datos);
    }
    archivo.close();
    ...
    if (cantidad < total) {
        InformacionArchivoPaciente* justo = nullptr;
        if (cantidad > 0) {
            justo = new InformacionArchivoPaciente[cantidad];
            memcpy(justo, pacientes, sizeof(InformacionArchivoPaciente) * cantidad);
        }
        delete[] pacientes;
        pacientes = justo; 
    }
...
inline void LiberarPacientes(InformacionArchivoPaciente*& pacientes) {
    delete[] pacientes;
    pacientes = nullptr;
}
    ...
    int totalConfig)
{
    ...
    int NumPacientes = ContarPacientesValidos ("Data/pacientes.csv"); 

    char pacientesProcesados[NumPacientes][20];
    for (int z = 0; z < NumPacientes; ++z) pacientesProcesados[z][0] = '\0';
    int cantPacientes = 0;
    ...
    for (int i = 0; i < static_cast<int>(static_cast<unsigned char>(sala.NumerodeMaquinas)); i++) {
        ...
        for (int j = 0; j < maq.NumeroDeMediciones; j++) {
            ...
            if (!yaExiste && cantPacientes < NumPacientes) {
...
int main()
{
    ...
    InformacionArchivoPaciente* pacientes = nullptr;
    ...
    {
        LeerArchivoConfiguracion(configuraciones, totalConfig, 5);
        LeerArchivoPacientes(totalPacientes);
        cout << "Los datos se guardaron correctamente" << endl;

        cout << "Se cargaron " << totalPacientes << " pacientes.\n";
